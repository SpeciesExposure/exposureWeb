...
/* ---- Hotspot Explorer: allow map row to reach its explicit 700px height ----
   The bslib fill chain makes every element flex-grow:1 into the viewport
   height.  Override only in the hotspot tab by setting flex-grow:0 so the
   chain auto-sizes from content instead, then force the inner grid rows. */
...
.bslib-grid[data-height="700px"] {
  grid-template-rows: 700px 320px !important;
  height: auto !important;
}
</style>
```
...
# Start map
lmap <- leaflet::leaflet(height = "700px", options = leaflet::leafletOptions(preferCanvas = TRUE)) |>
